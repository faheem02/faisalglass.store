<?php
/**
 * Add Sale Page - FIXED: ADVANCE / RECEIVED now user-editable
...
// Calculate all totals - FIXED: ADVANCE / RECEIVED is now user-editable
function calculateAllTotals() {
    ...
    // --- FIX: Always read user-entered received amount ---
    ...
    // For credit, we can set received to 0 but still allow user to change
    // No forced assignment now.
...
$(document).ready(function() {
    ...
    // Payment type change - now only affects visibility of bank field
    $('#payment_type').on('change', function() {
        if($(this).val() === 'bank') $('#bank_div').show();
        else $('#bank_div').hide();
        // Do NOT force received amount - let user decide
        calculateAllTotals();
    });
    
    // Listen to received amount changes
    $('#received_amount').on('input change', function() {
        calculateAllTotals();
    });
    
    $('#other_charges').on('keyup change', function() { calculateAllTotals(); });
