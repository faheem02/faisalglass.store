<?php
/**
 * Print Employee Ledger Statement Page
...
<head>
    ...
    <title>Employee Ledger Statement</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tb-salary { background: #dbeafe; color: #1e40af; border: 1px solid #bfdbfe; }
        .tb-payment { background: #d4edda; color: #155724; border: 1px solid #a3d9a5; }
        .tb-advance { background: #fff3cd; color: #856404; border: 1px solid #ffe69c; }
...
<div class="list-container" id="listContent">
    ...
    <div class="info-grid">
        ...
        <div class="info-item">
            ...
            <span class="info-value"><?php echo htmlspecialchars($employee['designation']) ?: '-'; ?></span>
        </div>
    </div>
    ...
    <div class="balance-row">
        <div class="balance-item">
            ...
            <div class="bal-value <?php echo $opening_balance > 0 ? 'bal-payable' : ($opening_balance < 0 ? 'bal-advance' : 'bal-neutral'); ?>">
                Rs <?php echo number_format(abs($opening_balance), 2); ?>
                <small><?php echo $opening_balance > 0 ? ' (Payable)' : ($opening_balance < 0 ? ' (Advance)' : ''); ?></small>
            </div>
        </div>
        <div class="balance-item">
            <div class="bal-label">Total Salary (Credits)</div>
            <div class="bal-value bal-payable">Rs <?php echo number_format($total_credit, 2); ?></div>
        </div>
        <div class="balance-item">
            <div class="bal-label">Total Payments (Debits)</div>
            <div class="bal-value bal-advance">Rs <?php echo number_format($total_debit, 2); ?></div>
        </div>
        <div class="balance-item">
            <div class="bal-label">Closing Balance</div>
            <div class="bal-value <?php echo $closing_balance > 0 ? 'bal-payable' : ($closing_balance < 0 ? 'bal-advance' : 'bal-neutral'); ?>">
                Rs <?php echo number_format(abs($closing_balance), 2); ?>
                <small><?php echo $closing_balance > 0 ? ' (Payable)' : ($closing_balance < 0 ? ' (Advance)' : ''); ?></small>
            </div>
        </div>
    </div>
    ...
    <table class="list-table">
        <thead>
            <tr>
                <th width="10%">Date</th>
                <th width="11%">Reference Type</th>
                <th width="8%">Ref ID</th>
                <th>Description</th>
                <th width="9%">Month</th>
                <th width="12%">Debit (Payment)</th>
                <th width="12%">Credit (Salary)</th>
        ...
        <tbody>
            <tr style="background:#f8f9fc;font-weight:600;">
                ...
                <td class="text-right"><strong><?php echo number_format(abs($opening_balance), 2); ?> <?php echo $opening_balance >= 0 ? '(Payable)' : '(Advance)'; ?></strong></td>
            </tr>
            <?php
            ...
            if($ledger_result && mysqli_num_rows($ledger_result) > 0):
                while($entry = mysqli_fetch_assoc($ledger_result)):
                    ...
                    switch($entry['reference_type']) {
                        ...
                        case 'SALARY': $badge_class = 'tb-salary'; $type_label = 'Salary'; break;
                        case 'PAYMENT': $badge_class = 'tb-payment'; $type_label = 'Payment'; break;
                        case 'ADVANCE': $badge_class = 'tb-advance'; $type_label = 'Advance'; break;
                        case 'ADJUSTMENT': $badge_class = 'tb-other'; $type_label = 'Adjustment'; break;
                        default: break;
                    }
            ?>
            <tr>
                ...
                <td class="text-center"><?php echo $entry['reference_id']; ?></td>
                <td><?php echo htmlspecialchars($entry['description']); ?></td>
                <td class="text-center"><?php echo $entry['month_year'] ? date('M Y', strtotime($entry['month_year'] . '-01')) : '-'; ?></td>
                ...
                <td class="text-right">
                    ...
                    elseif($running_balance < 0) echo number_format(abs($running_balance), 2) . ' (Advance)';
                    else echo '0.00';
                    ?>
                </td>
            </tr>
            <?php endwhile; else: ?>
            <tr><td colspan="8" class="text-center" style="padding:20px;">No ledger entries found for the selected period</td></tr>
            <?php endif; ?>
        </tbody>
        <tfoot>
            <tr class="table-footer">
                ...
                <td class="text-right">
                    ...
                    elseif($closing_balance < 0) echo number_format(abs($closing_balance), 2) . ' (Advance)';
                    else echo '0.00';
                    ?>
                </td>
            </tr>
